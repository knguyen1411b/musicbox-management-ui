...
export default function BookingQrModal({ booking, onClose }) {
    ...
    return (
        <div
            className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overscroll-contain"
            role="dialog"
            aria-modal="true"
        >
            <div className="bg-slate-900 w-full max-w-sm rounded-3xl p-8 border border-violet-500/30 shadow-2xl shadow-violet-500/20 flex flex-col items-center text-center space-y-6 max-h-[92vh] overflow-y-auto">
                {/* Mã đặt phòng */}
                <div>
                ...
                <button
                    type="button"
                    onClick={onClose}
                    className="w-full py-3 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white rounded-xl font-bold text-sm transition-colors mt-4"
                >
                    Đóng
                </button>
            </div>
