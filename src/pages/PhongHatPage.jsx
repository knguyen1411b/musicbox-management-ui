...
import { Search, Play, Trash2, ListMusic, Tv, Plus, Check } from 'lucide-react';
...
export default function PhongHatPage() {
    ...
    const videoPlaceholder = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ8ZtaITp1VBmbIWnrkRpTHCNuvuENfHefiDw&s";

    return (
        <div className="min-h-screen text-slate-100 p-4 md:p-6 font-sans selection:bg-[#8b5cf6]" style={{ backgroundColor: '#1e1b4b' }}>
                    ...
                    <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-4 shadow-xl">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <h1 className="text-2xl font-black tracking-wider bg-gradient-to-r from-white via-[#c4b5fd] to-[#8b5cf6] bg-clip-text text-transparent flex items-center gap-2">
                                OyAnGhFn
                                <span className="text-xs font-normal text-[#c4b5fd] border border-[#8b5cf6]/30 px-2 py-0.5 rounded-full backdrop-blur-sm">PRO</span>
                            </h1>
                            ...
                            >
                                {copied ? (
                                    ...
                                    <>
                                        ...
                                        <span className="opacity-0 group-hover:opacity-100 max-w-0 group-hover:max-w-[80px] overflow-hidden transition-all duration-300 ease-out whitespace-nowrap">Chia sẻ</span>
                                    </>
                                )}
                            </button>
                        </div>
                            ...
                            {mockSearchResults.map((song) => (
                                ...
                                >
                                    <div className="relative w-20 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-slate-800">
                                        <img src={videoPlaceholder} alt="thumbnail" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                                        <span className="absolute bottom-1 right-1 text-[10px] bg-black/60 px-1 rounded text-slate-300">{song.duration}</span>
                                    </div>

                                    <div className="flex flex-col justify-between flex-1 min-w-0">
                                        <h4 className="text-sm font-medium line-clamp-2 leading-snug group-hover:text-white transition-colors">{song.title}</h4>
                        ...
                        <div className="relative aspect-video rounded-xl overflow-hidden bg-black/40 border border-white/5 shadow-inner">
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-black/60 p-4 md:p-6 flex flex-col justify-between">
                                ...
                                <div className="flex items-start justify-between">
                                    <div>
                                        <span className="text-[10px] md:text-xs bg-[#8b5cf6] text-white px-2 py-0.5 rounded-md font-semibold tracking-wide uppercase shadow-md shadow-[#8b5cf6]/30">Đang phát</span>
                                        <h2 className="text-lg md:text-2xl font-bold mt-2 text-white drop-shadow-md">
                                            MƯA TRÊN BIỂN VẮNG <span className="text-[#c4b5fd] text-sm md:text-lg font-medium">*karaoke *tone nữ *ngọc lan</span>
                                        </h2>
                                    </div>
                    ...
                    <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-5 shadow-xl flex-1">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-bold text-base md:text-lg tracking-wide text-white">Hàng Chờ Bài Hát ({playlist.length})</h3>
                            </div>
                            <span className="text-xs text-[#c4b5fd] bg-[#6d28d9]/30 border border-[#8b5cf6]/30 px-2 py-1 rounded-lg">Mã phòng: P01</span>
                        </div>
                                ...
                                {playlist.map((song, index) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-3 min-w-0">
                                            <span className={`w-6 h-6 flex items-center justify-center rounded-lg text-xs font-bold ${index === 0 ? 'bg-[#8b5cf6] text-white' : 'bg-white/10 text-[#c4b5fd]'
                                                }`}>
                                                {index === 0 ? <Check className="w-3.5 h-3.5" /> : index + 1}
                                            </span>
                                            <div className="min-w-0">
                                                <p className={`text-sm font-medium truncate ${index === 0 ? 'text-white' : 'text-slate-200'}`}>
                                                    {song.title}
                                                </p>
                                                <p className="text-[11px] text-[#c4b5fd]/80 mt-0.5">Yêu cầu bởi: <span className="font-medium text-white/90">{song.singer}</span></p>
                                            </div>
                                        </div>

                                        <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                                            {index === 0 && (
                                                <span className="text-[10px] bg-[#6d28d9] border border-[#8b5cf6] text-white px-2 py-1 rounded-md font-medium">Tiếp theo</span>
                                            )}
